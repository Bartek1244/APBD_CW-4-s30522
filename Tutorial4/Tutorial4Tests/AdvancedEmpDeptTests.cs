...
public class AdvancedEmpDeptTests
{
    ...
    public void AllEmployeesShouldEarnMoreThan500()
    {
        ...
        Assert.True(result.SequenceEqual(emps));
    }
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Select(
                e1 => new {Employee = e1.EName, Manager = emps.First(e2 => e1.Mgr == e2.EmpNo).EName}
            );
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps.Select(
                e => new {e.EName, Total = e.Sal + (e.Comm ?? 0)}
            ); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Select(
                e => new {
                    e.EName, 
                    depts.First(d => d.DeptNo == e.DeptNo).DName, 
                    grades.First(g => g.Losal < e.Sal && g.Hisal > e.Sal).Grade
                }
            ); 
